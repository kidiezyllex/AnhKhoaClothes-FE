...
export default function ReturnDetailModal({
  ...
  const { data: detailData, isLoading } = useMyReturnDetail(
  ...
  const getStatusTimeline = (
    ...
    const steps = [
      { key: "CHO_XU_LY", label: "Chờ xử lý", date: createdAt },
      {
        key: "DA_HOAN_TIEN",
        label: "Đã hoàn tiền",
        date: status === "DA_HOAN_TIEN" ? updatedAt : null,
      },
      {
        key: "DA_HUY",
        label: "Đã hủy",
        date: status === "DA_HUY" ? updatedAt : null,
      },
    ];
  ...
  const handleCancelReturn = async () => {
    ...
    try {
      ...
      queryClient.invalidateQueries({
        queryKey: ["myReturn", (returnItem as any)?.id],
      });
